...
int CRS232::OpenUart(int port)
{
    ...
    const char * com[] = {"/dev/ttyS0","/dev/ttyS1","/dev/ttyS2","/dev/ttyS3"};   //901
    // const char * com[] = {"/dev/ttymxc6","/dev/ttymxc7","/dev/ttymxc5","/dev/ttymxc4"};   //702
...
void * CRS232::ProcessThreadFunction(void * arg)//接受数据的线程函数
{
    ...
    while(true)
    {
        ...
        if(len > 0)
        {
            ...
            SKY_TRACE_DEBUG("[SERIAL RECEIVE] Len is %d, content is: %s\n", len, buff);
            printf("[SERIAL RECEIVE] Len is %d, content is: %s\n", len, buff);
            for (i = 0; i < len; i++)
            {
                SKY_TRACE_DEBUG("0x%02x ", buff[i]);
